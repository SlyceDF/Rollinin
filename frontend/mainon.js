...
var level = 0;
var levelcolors = thisLevel['level']['colors'][0]
var data = thisLevel['level']['layout'][0]
...
var render = function () {
  ...
  document.body.querySelector('#dlist').innerHTML = dinum.toString() + ' ❖';
  document.body.querySelector('#blist').innerHTML = maxpercent.toString() + '% BEST';
	renderer.render(scene, camera);
	ball.update();
	percent = Math.ceil(
		Math.abs(ball.mesh.position.z) / data.length * 100);
	percent = percent > 100 ? 100 : percent;
  if (percent >= 100 && diama == true) {
    diam = true
  };
